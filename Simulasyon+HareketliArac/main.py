...
#ARACINSINIFIII
class Tofask(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.bizim_arac = pygame.image.load('pembearaba.png')
        self.image = self.bizim_arac
    ...
    def set_rotation(self):
        ...
        self.image = pygame.transform.rotozoom(self.original_image, self.angle, 0.025)
    ...
    def accelerate(self):
        if self.active and not self.backward:  # İleri hareket
            position = pygame.math.Vector2(self.rect.topright)
            position += self.forward * 5
            self.rect.topright = (position.x, position.y)
        elif self.active and self.backward:  # Geri hareket
            position = pygame.math.Vector2(self.rect.topright)
            position -= self.forward * 5  # Geri hareket için çıkarma
            self.rect.topright = (position.x, position.y)
    ...
    def update(self):
        ...
        self.get_rotation()
        self.accelerate()
BizimArac = pygame.sprite.GroupSingle(Tofask())
...
def main():
    ...
    while run:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:  # ARACIN YÖNÜ VE KLAVYE TUŞLARIYLA HAREKETİ
                    if event.key == pygame.K_RIGHT:
                        BizimArac.sprite.direction += 1
                    if event.key == pygame.K_LEFT:
                        BizimArac.sprite.direction -= 1
                    if event.key == pygame.K_UP:
                        BizimArac.sprite.active = True  # İleri hareket
                    if event.key == pygame.K_DOWN:
                        BizimArac.sprite.active = True
                        BizimArac.sprite.backward = True  # Geri hareket

            if event.type == pygame.KEYUP:
                    if event.key == pygame.K_UP:
                        BizimArac.sprite.active = False
                    if event.key == pygame.K_DOWN:
                        BizimArac.sprite.active = False
                        BizimArac.sprite.backward = False  # Geri hareketi durdur
                    if event.key == pygame.K_RIGHT:
                        BizimArac.sprite.direction -= 1
                    if event.key == pygame.K_LEFT:
                        BizimArac.sprite.direction += 1
        ...
        pencere.blit(bizim_arac, (1435, 805))

        BizimArac.draw(pencere)
        # kamyonun çizimi
